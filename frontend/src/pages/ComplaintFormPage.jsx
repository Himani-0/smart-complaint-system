...
// Field is defined OUTSIDE ComplaintFormPage to prevent focus loss on every keystroke
const Field = ({ label, name, required = true, errors, children }) => (
  <div>
    <label className="block text-sm font-medium text-gray-700 mb-1">
      {label} {required && <span className="text-red-500">*</span>}
    </label>
    {children}
    {/* Only show error here when no children (children handle their own errors) */}
    {!children && errors[name] && <p className="text-red-500 text-xs mt-1">{errors[name]}</p>}
  </div>
);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Full Name" name="name" errors={errors}>
              <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Your full name" className={`input-field ${errors.name ? 'border-red-400' : ''}`} />
              {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
            </Field>
            <Field label="Email" name="email" errors={errors}>
              <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="your@email.com" className={`input-field ${errors.email ? 'border-red-400' : ''}`} />
              {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
            </Field>
          </div>

          <Field label="Complaint Title" name="title" errors={errors}>
            <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Brief title of the issue (min. 5 chars)" className={`input-field ${errors.title ? 'border-red-400' : ''}`} />
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Field label="Category" name="category" errors={errors}>
              <select name="category" value={formData.category} onChange={handleChange} className={`input-field ${errors.category ? 'border-red-400' : ''}`}>
            ...
            <Field label="Location" name="location" errors={errors}>
              <input type="text" name="location" value={formData.location} onChange={handleChange} placeholder="e.g., Ghaziabad, Sector 5" className={`input-field ${errors.location ? 'border-red-400' : ''}`} />
